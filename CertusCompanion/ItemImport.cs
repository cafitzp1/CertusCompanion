...
namespace CertusCompanion
{
    ...
    class ItemImport
    {
        ...
        private int totalItemsOnImport;
        ...
        public ItemImport(List<WorkflowItem> currentItemsInDatabase)
        {
        ...
        public ItemImport(DateTime importDate, string fileName, string importType, int totalItemsOnImport, List<string> newItemsAdded, List<string> completedSinceLastImport)
        {
        ...
        public ItemImport Import(string csvFileName)
        {
            ...
            { 
                    ...
                    else 
                    {
                        try
                        {
                            ...
                            if (!DoesWorkflowInformationMatch(updateItem, importItem))
                            {
                                ...
                                // if an item is yellow after the import, it means the item was never changed on certus even though it was white and should have been
                                // the item wf information is now back to reflecting certus
                                if (wi.DisplayColor == "SpringGreen")
                                {
                                    wi.DisplayColor = "Yellow";

                                    wi.Note += $"<<Workflow Item data was not reflected on certus ({updateItem.Status} - {updateItem.AssignedToName}" +
                                        $" vs {wi.Status} - {wi.AssignedToName}). Item has been changed back to reflect Certus data>>";
                                }
                ...
                foreach (WorkflowItem item in CurrentItemsInDatabase)
                {
                    ...
                    if (!CurrentImportItems.Exists(i => i.DocumentWorkflowItemID == item.DocumentWorkflowItemID))
                    {
                        ...
                        // by default, these items should show the completed color which is gray. don't remove black or yellow
                        if (item.DisplayColor != "Black" && item.DisplayColor != "Yellow" && item.DisplayColor != "Gray")
